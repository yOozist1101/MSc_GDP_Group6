...
def main():
    ...
    input_files = [os.path.join(data_dir, f'Day {i:02d}.csv') for i in range(2, 8)]
    merged_file = os.path.join(output_dir, 'merged_all_days.csv')
    concat_files(file_list=input_files, output_file=merged_file)
    
    # Step 2: Match deviceId to POIs
    poi_file = os.path.join(data_dir, 'Target-point.csv')
    matched_file = os.path.join(output_dir, 'matched_trajectories.csv')
    ...
    cleaned_file = os.path.join(output_dir, 'cleaned_gps_data.csv')
    ...
    output_html = os.path.join(output_dir, 'trajectories_map.html')
    ...
    print(f"Final output saved to: {output_html}")
